...
class LatencyTracker:
    ...
    tick_to_decision: RollingQuantiles = field(
        default_factory=lambda: RollingQuantiles(20_000)
    )
    decision_to_send: RollingQuantiles = field(
        default_factory=lambda: RollingQuantiles(20_000)
    )
    send_to_ack: RollingQuantiles = field(
        default_factory=lambda: RollingQuantiles(20_000)
    )
    ack_to_fill: RollingQuantiles = field(
        default_factory=lambda: RollingQuantiles(20_000)
    )
    decision_to_fill: RollingQuantiles = field(
        default_factory=lambda: RollingQuantiles(20_000)
    )
    ...
    def _print_spike(self, tr: TraceRecord) -> None:
        ...
        """
        total = (
            (tr.ts_first_fill_ns - tr.ts_decision_ns)
            if (tr.ts_first_fill_ns and tr.ts_decision_ns)
            else 0
        )
        tick_to_decision = (
            (tr.ts_decision_ns - tr.ts_tick_ns)
            if (tr.ts_decision_ns and tr.ts_tick_ns)
            else 0
        )
        decision_to_send = (
            (tr.ts_send_start_ns - tr.ts_decision_ns)
            if (tr.ts_send_start_ns and tr.ts_decision_ns)
            else 0
        )
        send_to_ack = (
            (tr.ts_ack_ns - (tr.ts_send_done_ns or tr.ts_send_start_ns))
            if (tr.ts_ack_ns and (tr.ts_send_done_ns or tr.ts_send_start_ns))
            else 0
        )
        ack_to_fill = (
            (tr.ts_first_fill_ns - tr.ts_ack_ns)
            if (tr.ts_first_fill_ns and tr.ts_ack_ns)
            else 0
        )
