...
def test_duplicate_ack_does_not_break():
    ...
    ack = OrderEvent(type="order", intent_id="i1", exchange_order_id="ex1", status="ACK", ts_ns=2)
...
def test_duplicate_fill_does_not_double_count():
    oms = OMS()
    oms.on_intent(OrderIntent(type="intent", intent_id="i1", symbol="btcusdt", side="buy", qty=1.0, price=100, ts_ns=1))
    oms.on_order_event(OrderEvent(type="order",intent_id="i1", exchange_order_id="ex1", status="ACK", ts_ns=2))

    f1 = FillEvent(type="fill", intent_id="i1", exchange_order_id="ex1", qty=0.4, price=101.0, ts_ns=10)
    oms.on_fill(f1)
...
def test_out_of_order_fill_before_ack():
    ...
    oms.on_fill(FillEvent(type="fill", intent_id="i1", exchange_order_id="ex1", qty=1.0, price=99.5, ts_ns=5))

    # Then ACK arrives
    oms.on_order_event(OrderEvent(type="order", intent_id="i1", exchange_order_id="ex1", status="ACK", ts_ns=6))
